...
#search variable will give us all the dashboards.......................
    ...
    url=`curl -X  GET -H  "Authorization:Bearer $2" $1/api/search?folderIds=0&query=&starred=false`
	
	search=`echo $url | grep \""$1"\" | sed  "s/\:/\n/"  | grep -v \""$1"\"  | sed "s/\"\,//g"  | sed 's/["]*$//' | sed 's/[,]*$//'| sed 's/^[ \t]*//'  | sed s/\"// | sed -n "${title}"p`

	echo ${search}
		
	#searchvalue will have all the title name.....................................
       {
       searchvalue=`echo $search`  
       }||
       {
